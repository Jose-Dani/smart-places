...
String  dir = "smartplaces.esy.es",
        port = "80",
        protocol = "TCP",
        request = "/index.php?action=status";
...
void loop(){
  ...
  if(WSerial.find("OK"))  //message returned when connection established WEAK SPOT!! DOESN'T ALWAYS CONNECT
  {
  ...
  cmd += "Host:"+dir+"\r\n\r\n";        //test file on my web
  Serial.println("Sending: AT+CIPSEND=" + cmd.length());
